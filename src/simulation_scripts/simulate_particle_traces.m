% function gillespie = simulate_particle_traces(seq_length, varargin)
    ...
    function gillespie = simulate_particle_traces(tVec, varargin)
    close all
    % calculate simulation params
    seq_length = numel(tVec);
    deltaT = tVec(2) - tVec(1);
    % set defaults 
    alpha = 0; % MS2 rise time
    K = 3; % number of effective states
    w = 7; % time steps to transcribe gene
%     R = rand(K)* 1 / deltaT * 2; % transition rate matrix
%     R(eye(K)==1) = -sum(R.*~eye(K));
    R = [-.012 .015 .005;
          .01   -.03 .03;
         .002  .015  -.035];
    r_emission = [0 4 8];
    ...
    for i = 1:numel(varargin)
        if isstring(varargin{i})
            if ismember(varargin{i},{'K', 'w', 'R', 'r_emission','noise','pi0'})       
    ...
    for k = 1:seq_length
        ...
        i_start = find(times <= t_start,1,'last');        
        
        i_end = find(times <= t_end,1,'last');        
